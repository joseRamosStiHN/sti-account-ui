...
@Component({
  selector: 'app-various-operations-list',
  templateUrl: './various-operations-list.component.html',
  styleUrl: './various-operations-list.component.css'
})
export class VariousOperationsListComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.dataSource$ = this.transService
      .getAllTransactionByDocumentType(DocumentType.VARIOUS_OPERATIONS)
      .pipe(
  ...
  onSearch(): void {
    ...
    this.dataSource$ = this.transService
      ...
      .pipe(
        map((data) => {
          return data.filter(item => item.documentType === 5)
        }),
        map((data) => this.fillDataSource(data)),
        tap({
          error: (err) => {
  ...
  goToClient = () => {
    this.router.navigate(['/accounting/various-operations']);
  };

  onButtonClick(data: any) {
    this.router.navigate(['/accounting/various-operations', data.id]);
  }
      ...
      if (d) {
        this.transService.putAllTransaction(this.selectRows).subscribe({
          next: (data) => {
            ...
            this.messageToast = 'Transacciones confirmadas con éxito';
            this.showToast = true;
          ...
          error: (err) => {
            ...
            this.messageToast = 'Error al intentar confirmar Transacciones';
            this.showToast = true;
          },
        });
      }
