...
export class AccountService {
  ...
  private accounts$?: Observable<AccountAPIResponse[]>;

  getAllAccountCached(): Observable<AccountAPIResponse[]> {
    if (!this.accounts$) {
      this.accounts$ = this.httpClient
        .get<AccountAPIResponse[]>(this.apiURL + '/api/v1/accounts')
        .pipe(
          shareReplay(1),
          catchError((err) => {
