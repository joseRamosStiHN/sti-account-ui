...
import { typeToast, DocumentType } from 'src/app/modules/accounting/models/models';
...
export class ListCompaniesComponent implements OnInit {
  ...
  async deleteCompany(company: CompanieResponse) {
    ...
    try {
      ...
      if (hasTransactions) {
        this.toastType = typeToast.Error;
        this.messageToast = 'No se puede eliminar la empresa porque existen transacciones asociadas';
        this.showToast = true;
        return;
      }
    } catch (error) {
    ...
    this.companyService.deleteCompany(company.id, actionByUser).subscribe({
      ...
      error: (err) => {
        ...
        if (err.error?.error?.includes('TenantId')) {
          this.messageToast = 'Error: No se pudo identificar la organización. Por favor, seleccione una compañía nuevamente.';
